...
function sportGen() {
    ...
    <script type="text/javascript">
        ...
        function blockGen(data) {
            if (data == ''){
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    addMarker({lat:data[i].Latitude, lng:data[i].Longitude, name:data[i].Name, addr:data[i].Address, contact:data[i].Contact, web:data[i].Website, court:data[i].NumberFieldCourts, surface:data[i].FieldSurfaceType, age:data[i].FacilityAge, condition:data[i].FacilityCondition, upgrade:data[i].FacilityUpgradeAge, rooms:data[i].Changerooms});
                    if (i == 200){
                        break;
                    }
                }
        ...
        function addInfowindow(marker, pos) {
            ...
            google.maps.event.addListener(marker,'click',function() {
                var infowindow = new google.maps.InfoWindow({
                    content: '<div class="cus-InfoWindow">' +
                        '<h3 id="cus-info-title">'+pos.name+'</h3><hr>' +
                        '<p><i class="fas fa-map-marker-alt"></i> Address： '+pos.addr+'<br>'+
                        '<i class="fas fa-info"></i>&nbsp&nbsp Number of Courts： '+pos.court+'<br>'+
                        '<i class="fas fa-info"></i>&nbsp&nbsp Filed Surface： '+pos.surface+'<br>'+
                        '<i class="fas fa-info"></i>&nbsp&nbsp Facility Age： '+pos.age+'<br>'+
                        '<i class="fas fa-info"></i>&nbsp&nbsp Facility Condition： '+pos.condition+'<br>'+
                        '<i class="fas fa-info"></i>&nbsp&nbsp Facility Upgrade Age： '+pos.upgrade+'<br>'+
                        '<i class="fas fa-info"></i>&nbsp&nbsp Change Rooms： '+pos.rooms+'<br>'+
                        '</p></div>'
                });
                infowindow.open(map,marker);
                map.panTo(marker.getPosition());
        ...
        function initMarker() {
            ...
            if ($count == 200) {break;}
            // add marker for each location
            echo 'addMarker({lat:'.$rows->Latitude.',lng:'.$rows->Longitude.',name:"'.$rows->Name.'",addr:"'.$rows->Address.'",contact:"'.$rows->Contact.'",web:"'.$rows->Website.'" ,court:"'.$rows->NumberFieldCourts.'",surface:"'.$rows->FieldSurfaceType.'",age:"'.$rows->FacilityAge.'" ,condition:"'.$rows->FacilityCondition.'" ,upgrade:"'.$rows->FacilityUpgradeAge.'",rooms:"'.$rows->Changerooms.'" });';
            $count++;
        } ?>
        }
