...
class WPRM_SC_Time extends WPRM_Template_Shortcode {
	...
	public static function shortcode( $atts ) {
		...
		switch ( $atts['type'] ) {
			case 'prep':
				$output = self::get_time( 'prep_time', $recipe->prep_time(), $atts );
				break;
			case 'cook':
				$output = self::get_time( 'cook_time', $recipe->cook_time(), $atts );
				break;
			case 'total':
				$output = self::get_time( 'total_time', $recipe->total_time(), $atts );
				break;
			case 'custom':
				$output = self::get_time( 'custom_time', $recipe->custom_time(), $atts );
				break;
		}
	 ...
	 */
	private static function get_time( $type, $time, $atts ) {
