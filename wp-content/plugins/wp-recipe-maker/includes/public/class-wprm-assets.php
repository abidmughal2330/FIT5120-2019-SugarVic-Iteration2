...
class WPRM_Assets {
	...
	public static function enqueue() {
		...
		wp_enqueue_style( 'wprm-public', WPRM_URL . 'dist/public-' . $template_mode . '.css', array(), WPRM_VERSION, 'all' );
		wp_enqueue_script( 'wprm-public', WPRM_URL . 'dist/public-' . $template_mode . '.js', array( 'jquery' ), WPRM_VERSION, true );
	...
	public static function enqueue_admin() {
		...
		if ( 'toplevel_page_wprecipemaker' === $screen->id ) {
			...
			/* TODO Wait for Manage page to be ready.
			wp_enqueue_style( 'wprm-admin-manage', WPRM_URL . 'dist/admin-manage.css', array(), WPRM_VERSION, 'all' );
			wp_enqueue_script( 'wprm-admin-manage', WPRM_URL . 'dist/admin-manage.js', array( 'wprm-admin' ), WPRM_VERSION, true );

			// Localize script.
			$taxonomies = WPRM_Taxonomies::get_taxonomies( true );
			$terms = array();

			foreach ( $taxonomies as $key => $labels ) {
			...
			wp_localize_script( 'wprm-admin-manage', 'wprm_admin_manage', array(
		...
		$wprm_admin = array(
			...
			'endpoints' => array(
				...
				'nutrition' => get_rest_url( null, 'wp-recipe-maker/v1/nutrition' ),
		...
		wp_localize_script( 'wprm-admin', 'wprm_admin', $wprm_admin );
	}
