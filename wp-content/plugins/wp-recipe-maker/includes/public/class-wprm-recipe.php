...
class WPRM_Recipe {
	...
	public function get_data_manage() {
		...
		$recipe['tags']['ingredient'] = $this->tags( 'ingredient' );
	...
	public function author() {
		switch ( $this->author_display() ) {
			...
			case 'same':
				...
				if ( $name && $link && WPRM_Addons::is_active( 'premium' ) ) {
					return '<a href="' . esc_attr( $link ) . '" target="' . $target . '">' . $name . '</a>';
				} else {
					return $name;
				}
			default:
	...
	public function in_collection( $collection_id = 'inbox' ) {
		...
		$filtered_items = array_filter( $collection['items']['0-0'], function( $item ) {
			return $this->id() === $item['recipeId'];
		});
		...
		foreach ( $instructions as $group_index => $group ) {
			...
			foreach ( $group_instructions as $index => $instruction ) {
				...
				if ( isset( $instruction['image'] ) && $instruction['image'] ) {
					$thumb = wp_get_attachment_image_src( $instruction['image'], $size );
