...
class WPRM_Template_Manager {
	...
	public static function init() {
		add_action( 'wp_head', array( __CLASS__, 'snippet_css' ) );
	...
	/**
	 * Add template CSS to head.
	 *
	 * @since	4.0.0
	 */
	public static function snippet_css() {
		if ( 'modern' === WPRM_Settings::get( 'recipe_template_mode' ) && WPRM_Settings::get( 'recipe_snippets_automatically_add_modern' ) ) {
			if ( ! is_feed() && ! is_front_page() && is_singular() && is_main_query() ) {
				$snippet_template = self::get_template_by_type( 'snippet' );
				$snippet_css = self::get_template_css( $snippet_template );

				if ( $snippet_css ) {
					echo '<style type="text/css">' . $snippet_css . '</style>';
				}
			}
		}
	}
