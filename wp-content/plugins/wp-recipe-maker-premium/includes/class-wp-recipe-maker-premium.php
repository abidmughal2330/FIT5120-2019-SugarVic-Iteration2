...
class WP_Recipe_Maker_Premium {
	...
	private function define_constants() {
		define( 'WPRMP_VERSION', '4.3.4' );
		define( 'WPRMP_CORE_VERSION_REQUIRED', '4.2.0' );
	...
	public function admin_notice_missing_core() {
		if ( ! defined( 'WPRM_VERSION' ) ) {
			...
			echo '<br/><a href="https://wordpress.org/plugins/wp-recipe-maker/" target="_blank">WP Recipe Maker</a>';
			echo '</p></div>';
		}
	}
	...
	public function admin_notice_required_version() {
		...
		echo '<br/><a href="https://wordpress.org/plugins/wp-recipe-maker/" target="_blank">WP Recipe Maker ' . esc_html( WPRMP_CORE_VERSION_REQUIRED ) . '</a>';
		echo '</p></div>';
	}
	...
	private function load_dependencies() {
		...
		require_once( WPRMP_DIR . 'includes/public/class-wprmp-nutrition-label.php' );
		require_once( WPRMP_DIR . 'includes/public/class-wprmp-user-rating.php' );

		// Admin.
		if ( is_admin() ) {
			require_once( WPRMP_DIR . 'includes/admin/class-wprmp-manage.php' );
			require_once( WPRMP_DIR . 'includes/admin/class-wprmp-modal.php' );
		}
	}
